...
if( $argc == 5 )
{
    $_skipFoldersCommon = array('web-inf', 'tmp', '.svn', 'sqls', 'logs', 'project');
	$_folders = array('admin', 'front', 'task', 'test');
	$_curFolders = array_diff($_folders, array($argv[1]));
    switch ($argv[1])
    {
		case 'front':
		case 'admin':
		case 'task':
		case 'test':
			$_skipFolders = array_merge($_skipFoldersCommon, $_curFolders);
			$_skipFiles   = array();
			break;

    }
    $a = new AssemblyBuilder($_skipFolders, $_skipFiles);
    $a->run($argv[2],$argv[3],$argv[4]);
}
else
{
    echo "Usage: /usr/local/bin/php build_includes <sub_project> <root_path> <outfile> <cache_key>\n";
}
...
{/*{{{*/
    ...
    private static $_skipFolders = array();
    private static $_skipFiles= array();
    private static $_fileNameTemplate = array('php');
    
    public function AssemblyBuilder($_skipFolders, $_skipFiles)
    {/*{{{*/
        self::$_skipFolders = $_skipFolders;
        self::$_skipFiles   = $_skipFiles;
    }/*}}}*/
