...
class QFrameLoader
{
    ...
    protected static function getClassPath()
    {
        ...
        {
            $classpath = apc_fetch("fw:autoload:application:1369408040");
            if ($classpath) return $classpath;

            $classpath = self::getClassMapDef();
            apc_store("fw:autoload:application:1369408040",$classpath);
        }
        else if(function_exists("eaccelerator_get"))
        {
            $classpath = eaccelerator_get("fw:autoload:application:1369408040");
            if ($classpath) return $classpath;

            $classpath = self::getClassMapDef();
            eaccelerator_put("fw:autoload:application:1369408040",$classpath);
        }
        else
        {
            $classpath = self::getClassMapDef();
    ...
    {
        return array(
            			"QFrameLoader" => 			"/home/q/php/QFrame/Loader.php",
