...
function init()
{
  mkdir -p $LOCAL_TMP_DIR;chmod 777 $LOCAL_TMP_DIR;chmod 777 $LOCAL_TMP_DIR/..
}
...
function get_online_src()
{
    ...
    echo  "      线上代码路径为 $host:$ONLINE_TMP_DIR/$online_src"
    echo  "      本地代码路径为 $LOCAL_TMP_DIR/$online_src"
		exit 1
	fi
...
function upload_src_to()
{
	...
	$SSH $host "$EXPORT_LANGUAGE;tar xvfz $uploaded_src_tgz -C $realdst" 2>&1 | sed -e 's/^/   /'
...
function sudo_ssh_run()
{
	...
	ssh -t $host "sudo -p $sudo_password $cmd"
	result=$?
	if [ $result -ne 0 ]
	then
